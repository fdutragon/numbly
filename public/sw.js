...
self.addEventListener("fetch", (event) => {
  ...
  if (
    !url.pathname.match(/\.(png|jpg|jpeg|gif|svg|ico|webp|woff|woff2)$/) &&
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    event.waitUntil(
      self.registration
        .showNotification(data.title || "Numbly", options)
        .then(() => {
...
self.addEventListener("notificationclick", (event) => {
  ...
  if (jwt) {
    console.log(
      "🔑 Service Worker: JWT encontrado no clique, preparando autenticação...",
    );
