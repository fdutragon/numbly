...
async function testPushAuthFlow() {
  ...
  try {
    ...
    const checkResponse = await fetch(
      `${TEST_CONFIG.baseUrl}/api/auth/check-device-push`,
      {
        method: "POST",
        headers: {
    ...
    console.log(
      "   (Em produção, o service worker capturaria o JWT automaticamente)",
    );
    ...
    console.log(
      "   ⚠️  Para testar completamente, use o navegador e clique no push real\n",
    );

    // 3. Verificar se o endpoint de login via JWT está funcionando
    console.log("3. 🔧 Testando endpoint de login via JWT...");
    console.log(
      "   (Usando JWT de exemplo - falhará por ser inválido, mas testa a API)",
    );

    const jwtResponse = await fetch(
      `${TEST_CONFIG.baseUrl}/api/auth/login-jwt`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
