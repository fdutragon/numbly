  ...
  if (selectedFriend) {
    ...
    if (selectedFriend.numerologyData) {
      ...
      const friendPriorityOrder = [
        ...
        { key: 'Número do Aniversário', value: friendNums['Número do Aniversário'] },
      ...
      if (friendNumerologyInfo.length > 0) {
        ...
        if (friendNums['Caminho da Vida']) contextInfo += `\n- Caminho da Vida: ${friendNums['Caminho da Vida']}`;
        if (friendNums['Destino']) contextInfo += `\n- Destino: ${friendNums['Destino']}`;
        if (friendNums['Impulso da Alma']) contextInfo += `\n- Impulso da Alma: ${friendNums['Impulso da Alma']}`;
        if (friendNums['Personalidade']) contextInfo += `\n- Personalidade: ${friendNums['Personalidade']}`;
      }
    }
...
async function callGroqAPI(thread: ConversationThread, model: GroqModel): Promise<string> {
  const groqPayload = {
    model: model.name,
  ...
  try {
    console.log(`[Chat] 🤖 Usando modelo: ${model.name} (${model.description})`);
...
function generateFallbackResponse(prompt: string, numerologyContext?: any): string {
  const userName = numerologyContext?.userData?.firstName || 
  ...
  if (prompt.toLowerCase().includes("compatibilidade") || 
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🔍 Request mode: PWA=${isPWA}, UserAgent: ${userAgent.substring(0, 50)}...`);

    // Preparar contexto numerológico
    const userData = numerologyContext?.userData;
    const selectedFriend = numerologyContext?.selectedFriend;
    
    console.log("📊 Contexto recebido:", JSON.stringify(numerologyContext, null, 2));

    // Obter ou criar thread de conversa
    const thread = getConversationThread(threadId, userData);
    console.log(`🧵 Thread: ${thread.id} (${thread.messages.length - 1} mensagens anteriores)`);
    ...
    try {
      ...
      console.error(`❌ Erro no modelo principal ${modelUsed.name}:`, primaryError);
      ...
      for (let i = 1; i < GROQ_MODELS.length; i++) {
        ...
        try {
          ...
          console.log(`❌ Modelo fallback ${fallbackModel.name} também falhou:`, fallbackError);
        }
      }
    ...
    const responseTime = Date.now() - startTime;
    console.log(`⚡ Resposta gerada em ${responseTime}ms (Thread: ${thread.id})`);
