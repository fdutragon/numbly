...
const CancelRecoverySchema = z.object({
  scheduledEmailIds: z.array(z.string()).min(1, 'IDs de emails são obrigatórios')
});
...
async function checkEmailExists(userEmail: string) {
  try {
    ...
    console.log(`🔍 Resultado: ${targetEmail} ${exists ? 'JÁ EXISTE' : 'NÃO EXISTE'} na audiência`);
    ...
    if (error.statusCode === 404 || error.message?.includes('not found')) {
      console.log(`🔍 Resultado: ${getTestEmail(userEmail)} NÃO EXISTE na audiência (404)`);
      return {
        exists: false,
        contact: null,
...
function personalizeTemplate(templateKey: keyof typeof RECOVERY_EMAIL_TEMPLATES, variables: Record<string, string>) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`Iniciando agendamento de emails de recuperação para ${userEmail} -> ${targetEmail}`);
    ...
    if (existsCheck.exists) {
      console.log(`⚠️ Email ${userEmail} -> ${targetEmail} já existe na audiência. Pulando agendamento.`);
      
      return NextResponse.json({
        success: true,
        alreadyExists: true,
        contact: existsCheck.contact,
        message: 'Email já existe na audiência. Emails de recuperação não foram reagendados.',
        scheduledCount: 0,
        totalEmails: 0,
        recoveryEmails: [],
    ...
    console.log(`Usuário ${userEmail} -> ${targetEmail} adicionado à audiência:`, contactResult);

    // Agendar emails individuais de recuperação
    const now = new Date();
    const emailResults = [];
    const firstName = userName.split(' ')[0];
    const defaultLoginLink = loginLink || `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/login`;
    ...
    for (const emailConfig of RECOVERY_SCHEDULE) {
      try {
        ...
        console.log(`📧 Agendando ${emailConfig.template} para ${scheduledTime.toISOString()}`);
        ...
        if (emailResult.error) {
          console.error(`Erro ao agendar ${emailConfig.template}:`, emailResult.error);
          throw new Error(`Falha ao agendar ${emailConfig.template}: ${emailResult.error.message}`);
        }
        ...
        console.log(`✅ ${emailConfig.template} agendado com ID: ${emailResult.data?.id}`);
        
      } catch (emailError: any) {
        console.error(`Erro ao agendar email ${emailConfig.template}:`, emailError);
    ...
    console.log(`✅ Processo concluído: ${successfulEmails.length}/${RECOVERY_SCHEDULE.length} emails agendados`);
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    console.log(`Iniciando cancelamento de ${scheduledEmailIds.length} emails agendados`);
    ...
    for (const emailId of scheduledEmailIds) {
      ...
      try {
        ...
        console.error(`❌ Erro de rede ao cancelar email ${emailId}:`, err.message);
        cancelResults.push({
          emailId,
          success: false,
    ...
    console.log(`✅ Processo de cancelamento concluído: ${successfulCancellations.length}/${scheduledEmailIds.length} emails cancelados`);
