...
export async function POST(request: NextRequest) {
  try {
    ...
    setTimeout(async () => {
      try {
        const insights = await generateEmotionalInsights(data.userId, personalDay);
        if (insights) {
...
function analyzeEmotionalPatterns(cycles: any[]) {
  ...
  Object.entries(moodsByPersonalDay).forEach(([day, moods]) => {
    ...
    const averageEnergy = energyByPersonalDay[parseInt(day)]
      .reduce((a, b) => a + b, 0) / energyByPersonalDay[parseInt(day)].length;
  ...
  return {
    personalDayPatterns: personalDayPatterns.sort((a, b) => a.personalDay - b.personalDay),
    weeklyTrends,
    activityAnalysis,
...
function analyzeWeeklyTrends(cycles: any[]) {
  ...
  return Object.entries(weeklyData).map(([week, data]) => ({
    week,
    averageEnergy: data.energy.reduce((a, b) => a + b, 0) / data.energy.length,
...
function analyzeActivityCorrelation(cycles: any[]) {
  ...
  const avgEnergyGeneral = cycles.reduce((sum, c) => sum + c.energy, 0) / cycles.length;
  const avgEnergyMeditation = withMeditation.length > 0 
    ? withMeditation.reduce((sum, c) => sum + c.energy, 0) / withMeditation.length 
    : 0;
  const avgEnergyJournal = withJournal.length > 0
    ? withJournal.reduce((sum, c) => sum + c.energy, 0) / withJournal.length
    : 0;
  const avgEnergyBoth = withBoth.length > 0
  ...
  return {
    ...
    meditationBoost: Math.round((avgEnergyMeditation - avgEnergyGeneral) * 10) / 10,
...
function generatePatternInsights(patterns: any[]): string[] {
  ...
  patterns.forEach(pattern => {
    ...
    if (averageEnergy >= 8) {
      insights.push(`Seus dias pessoais ${personalDay} são especialmente energéticos (${averageEnergy}/10)`);
    }
    
    if (averageEnergy <= 4) {
      insights.push(`Dias pessoais ${personalDay} tendem a ser mais introspectivos (energia ${averageEnergy}/10)`);
    }
    
    if (dominantMood === 'PEACEFUL') {
      insights.push(`Você encontra mais paz nos seus dias pessoais ${personalDay}`);
    }
    
    if (dominantMood === 'ANXIOUS') {
      insights.push(`Dias pessoais ${personalDay} podem trazer mais ansiedade - pratique autocuidado`);
    }
    
    if (dominantMood === 'REFLECTIVE') {
      insights.push(`Seus dias ${personalDay} são naturalmente reflexivos - aproveite para meditação`);
    }
  });
...
async function generateEmotionalInsights(userId: string, personalDay: number) {
  try {
    ...
    const recommendations = {
      'ANXIOUS': 'Pratique respiração profunda e meditação nos dias ' + personalDay,
      'PEACEFUL': 'Aproveite a serenidade dos dias ' + personalDay + ' para práticas espirituais',
      'ENERGETIC': 'Use a alta energia dos dias ' + personalDay + ' para manifestação',
      'REFLECTIVE': 'Dedique tempo extra para journaling nos dias ' + personalDay,
      'MELANCHOLIC': 'Seja gentil consigo mesmo nos dias ' + personalDay + ' - é natural sentir-se introspectivo',
      'GRATEFUL': 'Continue cultivando gratidão nos seus dias ' + personalDay + ' - isso amplifica sua vibração',
      'INSPIRED': 'Capture suas inspirações nos dias ' + personalDay + ' - elas são mensagens da alma',
      'CONFUSED': 'Confusão nos dias ' + personalDay + ' é normal - confie que a clareza virá',
      'BALANCED': 'Mantenha o equilíbrio dos dias ' + personalDay + ' como âncora para outros dias'
    };

    return {
      pattern,
      recommendation: recommendations[dominantMood as keyof typeof recommendations] || 'Continue observando seus padrões com compaixão'
    };
  } catch (error) {
    console.error('Erro ao gerar insights emocionais:', error);
    return null;
