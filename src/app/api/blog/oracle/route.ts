...
function getCurrentLunarPhase(): string {
  ...
  const daysSinceKnown = Math.floor((today.getTime() - knownNewMoon.getTime()) / (1000 * 60 * 60 * 24));
...
async function generateOracleMessage(context: {
  ...
  const { question, mood, personalDay, lunarPhase, cosmicAlignment, type } = context;
  ...
  if (question) {
    ...
    if (mood === 'PEACEFUL') {
      response = 'sua paz interior é um presente para o mundo. Mantenha essa serenidade - ela é a base de toda sabedoria.';
    } else if (mood === 'ENERGETIC') {
      response = 'sua energia vibrante é um portal de manifestação. Use essa força para materializar seus sonhos mais elevados.';
    } else {
      response = 'este momento é sagrado. Permita-se sentir profundamente - cada emoção é um professor disfarçado.';
    }
  }
...
function generateFutureLetter(personalDay: number, cosmicAlignment: string): string {
  const futurePhrases = [
  ...
  const phrase = futurePhrases[Math.floor(Math.random() * futurePhrases.length)];
