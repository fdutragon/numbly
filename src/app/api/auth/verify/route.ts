  ...
  try {
    ...
    if (!magicToken) {
      logSecurityEvent(
        "SUSPICIOUS",
        securityContext,
        `Invalid magic token: ${validToken}`,
      );
    ...
    if (!magicToken.expiresAt || magicToken.expiresAt < new Date()) {
      logSecurityEvent(
        "SUSPICIOUS",
        securityContext,
        `Expired magic token: ${validToken}`,
      );
    ...
    if (magicToken.used) {
      logSecurityEvent(
        "SUSPICIOUS",
        securityContext,
        `Already used magic token: ${validToken}`,
      );
      return NextResponse.json(
    ...
    const deviceId = magicToken.email
      .replace("device_", "")
      .replace("@numbly.local", "");
    ...
    logSecurityEvent(
      "AUTH_SUCCESS",
      securityContext,
      `Magic link verified for user: ${userDevice.user.id}`,
    );
