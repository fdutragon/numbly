  ...
  try {
    ...
    if (!user) {
      ...
      return NextResponse.json({
        success: true,
        message:
          "Se os dados estiverem corretos, você receberá uma notificação",
      });
    }
    ...
    if (pushSubscriptions.length > 0) {
      ...
      await fetch(
        `${process.env.NEXTAUTH_URL || "http://localhost:3001"}/api/push/send`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            actions: [{ action: "recover", title: "🚪 Acessar Conta" }],
          }),
        },
      );
    }
