...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const normalizedIp = securityContext?.ip
      ? normalizeIp(securityContext.ip)
      : null;
    let userDevices: any[] = [];
    ...
    for (const sub of pushSubscriptions) {
      try {
        console.log(
          "[CHECK_DEVICE] Tentando enviar push para subscription:",
          sub.id,
        );
        ...
        if (pushResult) {
          pushSentCount++;
          console.log(
            "[CHECK_DEVICE] Push enviado com sucesso para subscription:",
            sub.id,
          );
        } else {
          console.error(
            "[CHECK_DEVICE] Falha ao enviar push para subscription:",
            sub.id,
          );
        }
      } catch (pushError) {
        console.error(
          "[CHECK_DEVICE] Erro ao enviar push automático:",
          pushError,
        );
      }
    }
    ...
    logSecurityEvent(
      "AUTH_SUCCESS",
      securityContext,
      `Auto-push sent to ${pushSentCount} devices for IP: ${normalizedIp}`,
    );
