...
export async function POST(
  req: NextRequest,
): Promise<NextResponse<PushResponse>> {
  ...
  try {
    ...
    if (!isUserAdmin) {
      logSecurityEvent(
        "SUSPICIOUS",
        securityContext,
        "Tentativa não autorizada de envio de push notification",
      );
      return NextResponse.json<PushResponse>(
        {
          success: false,
    ...
    if (
      !checkRateLimit(pushKey, PUSH_RATE_LIMIT.window, PUSH_RATE_LIMIT.max, {
        allowLocalhost: true,
      })
    ) {
      logSecurityEvent(
        "RATE_LIMITED",
        securityContext,
        "Push notification rate limit exceeded",
      );
      return NextResponse.json<PushResponse>(
        {
          success: false,
          ...
          message:
            "Limite de envio de notificações excedido. Tente novamente em 5 minutos.",
        },
        { status: 429 },
      );
    }
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        logSecurityEvent(
          "SUSPICIOUS",
          securityContext,
          `Invalid push data: ${errorMessages}`,
        );
    ...
    console.log(
      `🎯 Enviando push para ${eligibleSubscriptions.length} usuários ${sendToAll ? "(TODOS)" : "(FILTRADOS)"} ${testMode ? "(MODO TESTE)" : ""}`,
    );
    ...
    for (const sub of eligibleSubscriptions) {
      try {
        ...
        try {
          ...
          console.error(
            `❌ Erro ao parsear subscription ${sub.id}:`,
            parseError,
          );
          results.failed++;
        ...
        if (
          ...
          results.failed++;
          results.errors.push(
            `Subscription ${sub.id}: Missing required fields`,
          );
          continue;
        }
