...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`🎯 Iniciando envio de push para usuários anônimos (título: ${title})`);
    ...
    console.log(`🎯 Encontrados ${anonymousUsers.length} usuários anônimos elegíveis`);
    ...
    console.log(`📦 Processando ${anonymousUsers.length} notificações em ${totalBatches} lotes de ${batchSize}`);

    for (let i = 0; i < anonymousUsers.length; i += batchSize) {
      const batch = anonymousUsers.slice(i, i + batchSize);
      const batchNumber = Math.floor(i / batchSize) + 1;
      
      console.log(`📤 Processando lote ${batchNumber}/${totalBatches} (${batch.length} notificações)`);
      ...
      const batchPromises = batch.map(async (user) => {
        try {
          ...
          try {
            ...
            results.failed++;
            results.errors.push(`Parse error for user ${user.id}: ${parseError}`);
            return;
          }

          // Validate required fields
          if (!user.endpoint || !subscriptionData.keys?.p256dh || !subscriptionData.keys?.auth) {
