...
const aiPromptSchema = z.object({
  ...
  type: z.enum(["chat", "report"], {
    required_error: "Tipo de prompt é obrigatório",
  }),
...
async function generateReport(
  ...
  type: string,
  number?: number,
): Promise<{ report: string; cached: boolean; generatedAt: Date }> {
...
${
  userData.numerologyData
    ? `NÚMEROS NUMEROLÓGICOS:
${Object.entries(userData.numerologyData)
...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        logSecurityEvent(
          "SUSPICIOUS",
          securityContext,
          `Invalid AI prompt data: ${errorMessages}`,
        );
    ...
    if (
      !checkRateLimit(rateLimitKey, rateLimit.window, rateLimit.max, {
        allowLocalhost: true,
      })
    ) {
      logSecurityEvent(
        "RATE_LIMITED",
        securityContext,
        `AI ${validatedData.type} rate limit exceeded`,
      );
      return NextResponse.json<AIResponse>(
        {
          success: false,
