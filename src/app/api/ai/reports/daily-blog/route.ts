...
const energiasPorNumero = {
  1: {
    ...
    oportunidades:
      "assumir o controle, liderar equipes, iniciar empreendimentos",
  ...
  3: {
    ...
    oportunidades:
      "expressar talentos artísticos, comunicar ideias, inspirar outros",
  ...
  4: {
    ...
    oportunidades:
      "construir bases sólidas, organizar projetos, criar sistemas",
...
export async function POST(request: NextRequest) {
  try {
    ...
    const numeroSincronia =
      (numeroDestino + diaPessoal) % 9 === 0
        ? 9
        : (numeroDestino + diaPessoal) % 9;

    // Dados do dia pessoal
    const energiaDia =
      energiasPorNumero[diaPessoal as keyof typeof energiasPorNumero];
    const energiaDestino =
      energiasPorNumero[numeroDestino as keyof typeof energiasPorNumero];
