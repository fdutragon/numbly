...
import { notifyEmailIdsReady } from '@/lib/email-status';
...
interface GlobalCache {
  emailIdsCache?: Map<string, EmailCacheData>;
}
...
async function sendTikTokInitiateCheckoutEvent(email: string): Promise<void> {
  try {
    ...
    console.log(
      `[TikTok Pixel] Enviando evento InitiateCheckout para ${email} (hash: ${hashedEmail.substring(0, 8)}...)`
    );
    ...
    if (response.ok) {
      ...
      console.log(
        `[TikTok Pixel] ✅ Evento InitiateCheckout enviado com sucesso:`,
        result
      );
    } else {
    ...
    console.error(
      '[TikTok Pixel] Erro ao enviar evento InitiateCheckout:',
      error
    );
  }
}
...
async function sendMetaInitiateCheckoutEvent(email: string): Promise<void> {
  try {
    if (!process.env.META_PIXEL_ID || !process.env.META_CAPI_TOKEN) {
      console.log(
        '[Meta Pixel] Credenciais não configuradas, pulando tracking'
      );
      return;
    }
    ...
    console.log(
      `[Meta Pixel] Enviando evento InitiateCheckout para ${email} (hash: ${hashedEmail.substring(0, 8)}...)`
    );
    ...
    if (response.ok) {
      const result = await response.json();
      console.log(
        `[Meta Pixel] ✅ Evento InitiateCheckout enviado com sucesso:`,
        result
      );
    } else {
      const errorText = await response.text();
      console.error(
        `[Meta Pixel] ❌ Erro ao enviar evento:`,
        response.status,
        errorText
      );
    }
    ...
    console.error(
      '[Meta Pixel] Erro ao enviar evento InitiateCheckout:',
      error
    );
  }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[APPMAX] Dados sanitizados:', {
        ...
        document: sanitizedData.document
          ? sanitizedData.document.substring(0, 3) + '...'
          : 'vazio',
    ...
    if (sanitizedData.document && sanitizedData.document.length >= 11) {
      ...
      customerPayload.document = cleanDocument.replace(
        /(\d{3})(\d{3})(\d{3})(\d{2})/,
        '$1.$2.$3-$4'
      );
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[APPMAX] Customer payload:', {
        ...
        document: customerPayload.document
          ? customerPayload.document.substring(0, 3) + '...'
          : 'não fornecido',
      });
    }
    ...
    if (!customerData || typeof customerData !== 'object') {
      console.error(
        '[APPMAX] ❌ Resposta inválida ao criar cliente:',
        customerData
      );
      return NextResponse.json(
        {
    ...
    if (!customerId) {
      console.error(
        '[APPMAX] ❌ ID do cliente não encontrado na resposta:',
        customerData
      );
      return NextResponse.json(
        {
          ...
          debug:
            process.env.NODE_ENV === 'development' ? customerData : undefined,
        },
        { status: 500 }
      );
    }
    ...
    if (!orderData || typeof orderData !== 'object') {
      console.error(
        '[APPMAX] ❌ Resposta inválida ao criar pedido:',
        orderData
      );
      return NextResponse.json(
        {
    ...
    if (!orderId) {
      console.error(
        '[APPMAX] ❌ ID do pedido não encontrado na resposta:',
        orderData
      );
      return NextResponse.json(
        {
    ...
    if (paymentMethod === 'credit-card') {
      ...
      do {
        ...
        if (
          typeof ccData.text === 'string' &&
          ccData.text.includes('Order is Cancelled') &&
          attempts === 1
        ) {
          console.log('[APPMAX] Pedido cancelado, tentando recriar...');

          // Recriar o pedido
          const newOrderRes = await fetch(
      ...
      if (!ccRes.ok) {
        let errorMessage =
          ...
          ccData.message ||
          (ccData.data &&
          typeof ccData.data === 'object' &&
          'message' in ccData.data
            ? (ccData.data as { message?: string }).message
            : undefined) ||
          (ccData.errors && Array.isArray(ccData.errors)
            ? ccData.errors.join(', ')
            : undefined) ||
        ...
        if (
          ...
          errorMessage.trim() === 'Order is Cancelled'
        ) {
        ...
        return NextResponse.json(
          {
            ...
            debug:
              process.env.NODE_ENV === 'development'
                ? { status: pixRes.status, statusText: pixRes.statusText }
                : undefined,
          },
          { status: 400 }
        );
      ...
      if (!sanitizedData.skipPixelsAndEmails) {
        ...
        console.log(
          '[PIXELS] ⏭️ Pulando envio de pixels PIX (skipPixelsAndEmails=true)'
        );
      }
      ...
      if (!sanitizedData.skipPixelsAndEmails) {
        Promise.resolve().then(async () => {
          try {
            ...
            const emailForScheduling =
              sanitizedData.email === 'felipedutra@outlook.com'
            ...
            if (resendRes.ok) {
              ...
              if (
                resendData.scheduledEmailIds &&
                Array.isArray(resendData.scheduledEmailIds)
              ) {
                const notified = notifyEmailIdsReady(
                  orderId,
                  resendData.scheduledEmailIds
                );
                ...
                if (!notified) {
                  ...
                  if (!globalCache.emailIdsCache)
                    globalCache.emailIdsCache = new Map();
        ...
        console.log(
          '[EMAILS] ⏭️ Pulando agendamento de emails (skipPixelsAndEmails=true)'
        );
      }
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (globalCache.emailIdsCache && globalCache.emailIdsCache.has(orderId)) {
      const cachedData = globalCache.emailIdsCache.get(orderId);
