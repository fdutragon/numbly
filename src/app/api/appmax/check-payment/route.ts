...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (userEmail.toLowerCase() === "felipedutra@outlook.com") {
      logWithTransaction(
        transactionId,
        "Modo desenvolvimento detectado - simulando pagamento aprovado",
      );
      ...
      logWithTransaction(
        transactionId,
        "Simulação: Status definido como aprovado",
      );
    } else {
      ...
      if (!accessToken) {
        addSecurityLog(
          ...
          "APPMAX_API_KEY não configurada",
          { transactionId },
        );
      ...
      try {
        const appmaxResponse = await fetch(
        ...
        if (!appmaxResponse.ok) {
          logWithTransaction(
            transactionId,
            `AppMax API erro: ${appmaxResponse.status} ${appmaxResponse.statusText}`,
          );
        ...
        logWithTransaction(transactionId, "Resposta AppMax recebida", {
    ...
    if (isCancelled) {
      logWithTransaction(
        transactionId,
        "Pagamento cancelado/expirado detectado",
        { status },
      );
    ...
    logWithTransaction(
      transactionId,
      "Pagamento confirmado, atualizando usuário para premium",
    );
    ...
    if (!user) {
      logWithTransaction(transactionId, "Usuário não encontrado", {
        userEmail,
      });
    ...
    if (!user.isPremium) {
      ...
      logWithTransaction(transactionId, "Usuário atualizado para premium", {
        userId: user.id,
      });
    } else {
      logWithTransaction(transactionId, "Usuário já é premium", {
        userId: user.id,
      });
    }
