 ...
 */
export async function GET(req: NextRequest): Promise<NextResponse<SecurityLogsResponse>> {
  ...
  try {
    ...
    if (!isAdmin(req)) {
      logSecurityEvent('SUSPICIOUS', securityContext, 'Tentativa de acesso não autorizado aos logs de segurança');
    ...
    logSecurityEvent('AUTH_SUCCESS', securityContext, 'Visualização de logs de segurança admin');
