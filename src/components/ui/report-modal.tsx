                ...
                <div>
                  ...
                  {reportData && (
                    <p className="text-xs text-gray-500">
                      {reportData.wordCount} palavras • {new Date(reportData.generatedAt).toLocaleDateString('pt-BR')}
                    </p>
                  )}
                </div>
              </div>
                ...
                >
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      <h3 className="text-red-800 font-semibold text-lg mb-2">Erro na geração do relatório</h3>
                ...
                >
                  <div className="prose prose-gray max-w-none prose-headings:text-gray-900 prose-p:text-gray-700 prose-p:leading-relaxed prose-strong:text-gray-900 prose-em:text-gray-600 prose-ul:text-gray-700 prose-ol:text-gray-700 prose-li:text-gray-700 prose-blockquote:text-gray-600 prose-blockquote:border-purple-300 prose-lg">
                    <ReactMarkdown 
                      ...
                      components={{
                        h1: ({children}) => <h1 className="text-3xl font-bold text-gray-900 mt-8 mb-6 first:mt-0 border-b border-gray-200 pb-4">{children}</h1>,
                        h2: ({children}) => <h2 className="text-2xl font-semibold text-gray-900 mt-8 mb-4 first:mt-0">{children}</h2>,
                        h3: ({children}) => <h3 className="text-xl font-semibold text-gray-900 mt-6 mb-3 first:mt-0">{children}</h3>,
                        h4: ({children}) => <h4 className="text-lg font-medium text-gray-800 mt-6 mb-2">{children}</h4>,
                        p: ({children}) => <p className="text-gray-700 leading-relaxed mb-6 text-base">{children}</p>,
                        strong: ({children}) => <strong className="font-semibold text-gray-900">{children}</strong>,
                        em: ({children}) => <em className="italic text-purple-600 font-medium">{children}</em>,
                        ul: ({children}) => <ul className="list-disc list-inside mb-6 text-gray-700 space-y-2 pl-4">{children}</ul>,
                        ol: ({children}) => <ol className="list-decimal list-inside mb-6 text-gray-700 space-y-2 pl-4">{children}</ol>,
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex items-center space-x-6 text-sm text-gray-500">
                        ...
                        <span className="flex items-center">
                          ...
                          {reportData.cached ? 'Relatório salvo' : 'Gerado agora'}
                        </span>
                      </div>
                      <div className="text-right text-sm text-gray-500">
                        <p className="font-medium">Relatório gerado por Numbly.Life</p>
