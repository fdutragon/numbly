...
export const intentionResponses = {
  payment: {
    basic:
      'Perfeito! O plano Básico é ideal para começar. Vou te direcionar para o checkout.',
    pro: 'Excelente escolha! O plano Pro tem todos os recursos avançados. Vou te direcionar para o checkout.',
    general:
      'Temos dois planos disponíveis: Básico (R$ 49/mês) e Pro (R$ 99/mês). Qual você gostaria de conhecer?',
  },
  email: {
    success:
      'Perfeito! Acabei de enviar todas as informações para o seu email. Verifique sua caixa de entrada em alguns minutos.',
    error:
      'Ops! Não consegui enviar o email. Pode me informar novamente seu endereço de email?',
    request:
      'Claro! Posso enviar todas as informações detalhadas para o seu email. Qual é o seu endereço de email?',
  },
  objection: {
    price:
      'Entendo sua preocupação com o preço. Vamos ver: quanto você gasta por mês tentando fazer tudo manualmente? A Clara se paga sozinha!',
    trust:
      'Sua confiança é importante para nós. Temos uma garantia de 30 dias - se não ficar satisfeito, devolvemos 100% do seu dinheiro.',
    timing:
      'Entendo que o timing pode não parecer ideal. Mas pense assim: cada dia que passa sem automação é receita perdida. Que tal começar com o plano Básico?',
  },
};
...
export function detectIntention(message: string): IntentionResult {
  ...
  if (
    ...
    lowerMessage.includes('não tenho') ||
    lowerMessage.includes('não posso') ||
    lowerMessage.includes('muito dinheiro') ||
  ...
  if (
    lowerMessage.includes('não confio') ||
    lowerMessage.includes('suspeito') ||
    lowerMessage.includes('golpe') ||
    lowerMessage.includes('confiável')
  ) {
    return {
      intention: 'objection',
      confidence: 0.9,
  ...
  if (
    ...
    lowerMessage.includes('assinar') ||
    lowerMessage.includes('plano') ||
    lowerMessage.includes('comprar')
  ) {
    const isProPlan =
      lowerMessage.includes('pro') || lowerMessage.includes('premium');
    return {
      intention: 'payment',
      confidence: 0.9,
      extractedData: {
  ...
  if (
    ...
    lowerMessage.includes('pontos fracos')
  ) {
    return {
      intention: 'qualify',
      confidence: 0.8,
      extractedData: {
        ...
        phone: lowerMessage.includes('telefone')
          ? 'telefone detectado'
          : undefined,
      },
    };
  ...
  // Detecta urgência
  if (
    lowerMessage.includes('preciso agora') ||
    lowerMessage.includes('urgente') ||
    lowerMessage.includes('imediato') ||
