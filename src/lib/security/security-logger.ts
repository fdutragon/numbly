...
export function getSecurityStats(): {
  ...
  securityLogs.forEach(log => { byLevel[log.level]++; });
  ...
  for (const log of securityLogs) {
    ...
    if (log.message && (log.message.includes('rate limit') || log.message.includes('too many requests'))) {
      rateLimitEntries++;
    }
...
export function searchSecurityLogs(
  criteria: {
    level?: SecurityLogEntry['level'];
    ip?: string;
    event?: string;
...
export function detectSuspiciousActivity(): {
  ...
  const now = Date.now();
  const oneHour = 60 * 60 * 1000;
  const recentLogs = securityLogs.filter(log => now - log.timestamp <= oneHour);
  const multipleFailedLogins = recentLogs.filter(log => log.event.includes('login') && log.level === 'warn');
  const ipCounts: Record<string, number> = {};
  recentLogs.forEach(log => { if (log.ip) ipCounts[log.ip] = (ipCounts[log.ip] || 0) + 1; });
  const suspiciousIPs = Object.keys(ipCounts).filter(ip => ipCounts[ip] > 50);
  const rateLimitViolations = recentLogs.filter(log => log.message.includes('rate limit') || log.message.includes('too many requests'));
