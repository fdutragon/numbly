...
function calculateRiskScore(
  ...
  if (origin && host && !origin.includes(host) && req.method === "POST")
    score += 20;
...
export async function authGuard(
  ...
  const blockThreshold =
    options.allowLocalhost && process.env.NODE_ENV === "development" ? 95 : 80;
...
export function logSecurityEvent(
  ...
  if (
    ...
    (event === "HIGH_RISK_BLOCKED" || event === "SUSPICIOUS")
  ) {
 ...
 */
export function checkEndpointThrottle(
  ip: string,
  endpoint: string,
  maxAttempts: number = 20,
): boolean {
  const ipLogs = securityLogs.get(ip) || [];
  const recentLogs = ipLogs.filter(
    (log) =>
...
export function createSecurityMiddleware(options: {
  ...
  return async function securityMiddleware(req: NextRequest) {
    ...
    if (options.rateLimit) {
      ...
      if (
        !checkRateLimit(key, options.rateLimit.window, options.rateLimit.max, {
          allowLocalhost: true,
        })
      ) {
